 ...
 * @param {string} [shortsFormat] - Optional Shorts output format ('original', 'vertical_blurred', 'vertical_crop', 'vertical_moderate').
...
export async function cutSegment(inputPath, start, end, outputPath, watermarkText, shortsFormat = 'original', copyrightBypass = false, watermarkFontSize = 18, autoCaptionsSrtPath = null, cpuFriendly = false, onProgress = null) {
    ...
    let srtPath = autoCaptionsSrtPath; // Pre-generated by CLI (cached per video)
    ...
    if (needsFilterScript) {
        ...
        try {
            ...
            // NOTE: FFmpeg 7.x prints "-filter_complex_script is deprecated, use
            // -/filter_complex <file> instead" but the option still works. Migrating
            // to inline -filter_complex risks Windows command-line length limits; left
            // as-is intentionally (see AUDIT_PROMPT.md / ERROR_LOG.md #8 for context).
            const renderArgs = buildCutArgs({
                inputPath,
                start,
                end,
                outputPath,
            ...
            await runFFmpeg(renderArgs, { onProgress, jobDir });
            writeJobLog(jobDir, `[FFmpeg] Rendering segment success.`);
            return { watermarked: canWatermark };
        } catch (e) {
            // On filter render failure, re-log the exact filter script content and
            // arg array that were used, so the job log is self-contained for
            // debugging without having to correlate with the pre-run log line above.
            writeJobLog(jobDir, `[FFmpeg] ERROR: Rendering failed: ${e.message}`);
            writeJobLog(jobDir, `[FFmpeg] Failed filter script content:\n${filterContent}`);
            ...
            // Per-clip temp files: scriptPath and textFilePath are derived from
            // outputPath (unique per clip), so they are safe to delete here.
            //
            // autoCaptionsSrtPath is intentionally NOT deleted: it is a SHARED,
            // caller-owned cache file (`video_cache/<videoId>_gemini.srt`) generated
            // once by the CLI (or server) and reused across all clips of a video.
            // Deleting it in this finally block (Bug #2, AUDIT_PROMPT.md) caused
            // clip 2+ to fail with `[Parsed_subtitles] Unable to open ... .srt`
            // because the SRT was gone after clip 1 completed. Lifecycle ownership
            // belongs to the caller, which cleans up after the whole clip loop.
